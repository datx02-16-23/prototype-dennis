...
com.dennisjonsson.annotation.log.ast.ASTLogger.instance(new com.dennisjonsson.annotation.log.ast.SourceHeader("QuickSortVisual",new String [] { "package com.dennisjonsson.visualization.test;","","import com.dennisjonsson.annotation.VisualClass;","import com.dennisjonsson.annotation.Visualize;","import com.dennisjonsson.annotation.markup.AbstractType;","import java.util.Arrays;","","@VisualClass","public class QuickSort {","","    public static void sort(int[] array) {","        quickSort(array, 0, array.length - 1);","    }","","    public static void quickSort(@Visualize(abstractType = 'array') int[] arr, int low, int high) {","        if (arr == null || arr.length == 0)","            return;","        if (low >= high)","            return;","        int middle = low + (high - low) / 2;","        int pivot = arr[middle];","        int i = low, j = high;","        while (i <= j) {","            while (arr[i] < pivot) {","                i++;","            }","            while (arr[j] > pivot) {","                j--;","            }","            if (i <= j) {","                int temp = arr[i];","                arr[i] = arr[j];","                arr[j] = temp;","                i++;","                j--;","            }","        }","        if (low < j)","            quickSort(arr, low, j);","        if (high > i)","            quickSort(arr, i, high);","    }","}"},null,new com.dennisjonsson.annotation.markup.DataStructure [] {  com.dennisjonsson.annotation.markup.DataStructureFactory.getDataStructure("array","int[]","QuickSort:arr")},new com.dennisjonsson.visualization.test.app.MyInterpreter()));
    ...
    public static void quickSort( int[] arr, int low, int high) {
        ...
        int pivot = eval("QuickSort:pivot", write("QuickSort:QuickSort:arr", arr[read("QuickSort:arr", 0, middle)], 0, 1), 0, new int[] { 23, 23 });
        int i = low, j = high;
        while (i <= j) {
            while (eval(null, arr[read("QuickSort:arr", 0, i)], 2, new int[] { 26, 26 }) < pivot) {
                i++;
            }
            while (eval(null, arr[read("QuickSort:arr", 0, j)], 2, new int[] { 29, 29 }) > pivot) {
                j--;
            }
            if (i <= j) {
                int temp = eval("QuickSort:temp", write("QuickSort:QuickSort:arr", arr[read("QuickSort:arr", 0, i)], 0, 1), 0, new int[] { 33, 33 });
                eval("QuickSort:arr[i]", arr[read("QuickSort:arr", 0, i)] = write("QuickSort:QuickSort:arr", arr[read("QuickSort:arr", 0, j)], 0, 0), 0, new int[] { 34, 34 });
                eval("QuickSort:arr[j]", arr[read("QuickSort:arr", 0, j)] = write("QuickSort:temp", temp, 1, 0), 0, new int[] { 35, 35 });
                i++;
                j--;
            }
        }
        if (low < j)
            quickSort(eval("QuickSort:arr", write("QuickSort:QuickSort:arr", arr, 1, 1), 3, new int[] { 41, 41 }), low, j);
        if (high > i)
            quickSort(eval("QuickSort:arr", write("QuickSort:QuickSort:arr", arr, 1, 1), 3, new int[] { 43, 43 }), i, high);
    }
