...
com.dennisjonsson.annotation.log.ast.ASTLogger.instance(new com.dennisjonsson.annotation.log.ast.SourceHeader("BubbleSortVisual",new String [] { "/*"," * To change this license header, choose License Headers in Project Properties."," * To change this template file, choose Tools | Templates"," * and open the template in the editor."," */","package com.dennisjonsson.visualization.test;","","import com.dennisjonsson.annotation.VisualClass;","import com.dennisjonsson.annotation.Visualize;","import com.dennisjonsson.annotation.markup.AbstractType;","","@VisualClass","public class BubbleSort {","","    public static void sort(@Visualize(abstractType = 'array') int intArray[]) {","        int n = intArray.length;","        int temp = 0;","        for (int i = 0; i < n; i++) {","            for (int j = 1; j < (n - i); j++) {","                if (intArray[j - 1] > intArray[j]) {","                    //swap the elements!","                    temp = intArray[j - 1];","                    intArray[j - 1] = intArray[j];","                    intArray[j] = temp;","                }","            }","        }","    }","}"},null,new com.dennisjonsson.annotation.markup.DataStructure [] {  com.dennisjonsson.annotation.markup.DataStructureFactory.getDataStructure("array","int[]","BubbleSort:intArray")},new com.dennisjonsson.visualization.test.app.MyInterpreter()));
    ...
    public static void sort( int intArray[]) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 1; j < (n - i); j++) {
                if (eval(null, intArray[read("BubbleSort:intArray", 0, j - 1)], 2, new int[] { 24, 24 }) > eval(null, intArray[read("BubbleSort:intArray", 0, j)], 2, new int[] { 24, 24 })) {
                    //swap the elements!
                    eval("BubbleSort:temp", temp = write("BubbleSort:BubbleSort:intArray", intArray[read("BubbleSort:intArray", 0, j - 1)], 0, 1), 0, new int[] { 26, 26 });
                    eval("BubbleSort:intArray[j - 1]", intArray[read("BubbleSort:intArray", 0, j - 1)] = write("BubbleSort:BubbleSort:intArray", intArray[read("BubbleSort:intArray", 0, j)], 0, 0), 0, new int[] { 27, 27 });
                    eval("BubbleSort:intArray[j]", intArray[read("BubbleSort:intArray", 0, j)] = write("BubbleSort:temp", temp, 1, 0), 0, new int[] { 28, 28 });
                }
            }
        }
    }
